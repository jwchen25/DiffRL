...
if __name__ == "__main__":
    ...
    if file_format == "poscar":
        ...
        with open(file_path, 'rb') as f_pkl:
            ase_atoms_list = pickle.load(f_pkl)
        atoms_list = [ase_to_atoms(_atoms) for _atoms in ase_atoms_list]
    elif file_format == "extxyz":
        ase_atoms_list = ase.io.read(file_path, index=':')
        atoms_list = [ase_to_atoms(_atoms) for _atoms in ase_atoms_list]
    else:
        raise NotImplementedError("File format not implemented", file_format)

    if file_format not in ["pkl", "extxyz"]:
